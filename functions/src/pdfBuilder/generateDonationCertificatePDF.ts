...
export const createDonationCertificateCH = async (url: string, data: any, translations: Map<string, string>) => {
	...
	docPdf.fontSize(12).text(`${data.currentDate}`, 100, 80);
	docPdf.moveDown();
	...
	docPdf
		.fontSize(12)
		.text(translations.get('donation-certificate.salutation') + `${data.personal.name} ${data.personal.lastname}`);
	docPdf.moveDown();
	docPdf
		.fontSize(12)
		.text(
...
export const addDocumentToUser = async (data: any, downloadUrl: any) => {
	...
	firestore
		.collection('users/' + data.entityId + '/donation-certificate')
		.doc(documentKey)
		.set(
			{
				created: dateString,
...
export const translationStub = async () => {
	...
	translations.set(
		'donation-certificate.confirmation-2',
		' Spenden in folgender Höhe erhalten zu haben (Details siehe Anhang):'
	);
	translations.set(
		'donation-certificate.information-1',
		'Die Stiftung Social Income ist auf der Spendenliste des Kantons Zürich aufgeführt und somit in allen Kantonen als gemeinnnützige Organisation anerkannt.'
	);
