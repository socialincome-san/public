...
import { MessageType, MESSAGE_FIRESTORE_PATH, Recipient, RECIPIENT_FIRESTORE_PATH, TwilioMessage } from '../../../shared/src/types';
...
export class TwilioIncomingMessageHandler {
    ...
    optInWhatsapp = async (recipientId: string) => {
        if (recipientId.length > 0) {
            const recipientDocRef = this.firestoreAdmin.doc<Recipient>(
    ...
    findRecipient = async (WaID: string): Promise<string> => {
        ...
        let recipientId = "";
        await this.firestoreAdmin.collection(`${RECIPIENT_FIRESTORE_PATH}`)
