...
export class FirestoreNewsletter {
	...
	addToMailchimp = async (
		dobject_old: Dictionary<String>,
		dobject: Dictionary<String>,
		fb_docid: String
	): Promise<any> => {
		client.setConfig({
		...
		try {
			...
			if (dobject_old.email != dobject.email) {
				const subscriberHash = crypto.createHash('md5').update(dobject_old.email.toLowerCase()).digest('hex');

				let response = client.lists.deleteListMember(MAILCHIMP_LIST_ID, subscriberHash);
				await response;
			}
			...
			let response = client.lists.setListMember(MAILCHIMP_LIST_ID, dobject.email, mc_data);
			await response;
			return Promise.resolve('Successful');
		} catch (error) {
	...
	registerTrigger = functions.firestore.document('newsletter-subscribers/{docId}').onWrite(async (change, context) => {
