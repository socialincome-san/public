...
describe('TwilioIncomingMessageProcessor', () =>  {
	...
	test('incoming opt-in message', async () =>  {
        ...
        await firestoreAdmin.collection(`${RECIPIENT_FIRESTORE_PATH}`)
        ...
        const foundRecipientId = await twilioIncomingMessageHandler.findRecipient(recipient.communication_mobile_phone.phone.toString());
        ...
        const recipientDocRef = firestoreAdmin.doc<Recipient>(
            `${RECIPIENT_FIRESTORE_PATH}`,
            recipientId
        );
