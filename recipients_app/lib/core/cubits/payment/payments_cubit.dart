...
class PaymentsCubit extends Cubit<PaymentsState> {
  ...
  PaymentsCubit({required this.recipient, required this.paymentRepository, required this.crashReportingRepository})
  ...
  Future<void> loadPayments() async {
    ...
    try {
      final payments = await paymentRepository.fetchPayments(recipientId: recipient.userId);

      emit(state.copyWith(status: PaymentsStatus.success, paymentsUiState: await _mapPaymentsUiState(payments)));
    } on Exception catch (ex, stackTrace) {
      crashReportingRepository.logError(ex, stackTrace);
      emit(state.copyWith(status: PaymentsStatus.failure, exception: ex));
    }
  }
  ...
  Future<void> confirmPayment(SocialIncomePayment payment) async {
    ...
    try {
      await paymentRepository.confirmPayment(recipient: recipient, payment: payment);

      final payments = await paymentRepository.fetchPayments(recipientId: recipient.userId);

      final paymentUiState = await _mapPaymentsUiState(payments);

      emit(state.copyWith(status: PaymentsStatus.updated, paymentsUiState: paymentUiState));
    } on Exception catch (ex, stackTrace) {
      crashReportingRepository.logError(ex, stackTrace);
      emit(state.copyWith(status: PaymentsStatus.failure, exception: ex));
    }
  ...
  Future<void> contestPayment(SocialIncomePayment payment, String contestReason) async {
    emit(state.copyWith(status: PaymentsStatus.loading));

    try {
      await paymentRepository.contestPayment(recipient: recipient, payment: payment, contestReason: contestReason);
  ...
  Future<PaymentsUiState> _mapPaymentsUiState(List<SocialIncomePayment> payments) async {
    ...
    for (int i = 0; i < payments.length; i++) {
      ...
      mappedPayments.add(MappedPayment(payment: currentPayment, uiStatus: paymentUiStatus));
    }

    final reversedMappedPayments = mappedPayments.reversed.toList();

    return PaymentsUiState(
      status: _getBalanceCardStatus(reversedMappedPayments, unconfirmedPaymentsCount),
  ...
  BalanceCardStatus _getBalanceCardStatus(List<MappedPayment> mappedPayments, int unconfirmedPaymentsCount) {
    ...
    if (mappedPayments.any(
      ...
      balanceCardStatus = BalanceCardStatus.onHold;
        ...
        mappedPayments.any((element) => element.uiStatus == PaymentUiStatus.recentToReview)) {
  ...
  NextPaymentData _getNextPaymentData(List<MappedPayment> mappedPayments) {
    final nextPayment = mappedPayments.firstWhereOrNull((element) => element.uiStatus == PaymentUiStatus.toBePaid);

    final previousPaymentDate =
        mappedPayments
            .firstWhereOrNull((element) => element.uiStatus != PaymentUiStatus.toBePaid)
            ?.payment
            .paymentAt
            ?.toDate();
