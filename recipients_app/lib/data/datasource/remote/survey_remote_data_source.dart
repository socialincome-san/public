...
class SurveyRemoteDataSource implements SurveyDataSource {
  ...
  const SurveyRemoteDataSource({required this.firestore});
  ...
  Future<List<Survey>> fetchSurveys({required String recipientId}) async {
    ...
    final surveysDocs =
        await firestore.collection(recipientCollection).doc(recipientId).collection(surveyCollection).get();

    for (final surveyDoc in surveysDocs.docs) {
      final survey = Survey.fromJson(surveyDoc.data());

      surveys.add(survey.copyWith(id: surveyDoc.id));
    }
