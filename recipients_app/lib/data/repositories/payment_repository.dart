...
class PaymentRepository {
  ...
  const PaymentRepository({required this.remoteDataSource, required this.demoDataSource, required this.demoManager});

  PaymentDataSource get _activeDataSource => demoManager.isDemoEnabled ? demoDataSource : remoteDataSource;

  Future<List<SocialIncomePayment>> fetchPayments({required String recipientId}) {
  ...
  Future<void> confirmPayment({required Recipient recipient, required SocialIncomePayment payment}) async {
    await _activeDataSource.confirmPayment(recipient: recipient, payment: payment);
  }
  ...
  Future<void> contestPayment({
    ...
    await _activeDataSource.contestPayment(recipient: recipient, payment: payment, contestReason: contestReason);
  }
}
