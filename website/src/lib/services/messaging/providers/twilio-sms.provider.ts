...
export class TwilioSmsProvider extends BaseService implements MessageProvider {
	...
	async send(request: SendMessageRequest): Promise<ServiceResult<SendMessageResponse>> {
		try {
			...
			this.logger.info('Twilio SMS sent successfully', { sid: message.sid, to: request.to });

			return this.resultOk({ externalId: message.sid });
		} catch (error) {
			this.logger.error('Failed to send Twilio SMS', { error, to: request.to });

			return this.resultFail(`Failed to send SMS: ${error instanceof Error ? error.message : String(error)}`);
		}
	}
	...
	private requireTwilioEnvVars(): ServiceResult<void> {
		if (!this.twilioAccountSid || !this.twilioApiKeySid || !this.twilioApiKeySecret || !this.twilioPhoneNumber) {
			return this.resultFail('Missing Twilio environment variables for SMS provider');
		}
