...
describe('messaging-selection', () => {
	...
	describe('toggleRow', () => {
		...
		test('all-matching mode: adds id to excludedIds when not excluded', () => {
			const initial = { mode: 'all-matching' as const, search: 'a', filters: {}, excludedIds: new Set<string>() };
		...
		test('all-matching mode: removes id from excludedIds when excluded', () => {
			const initial = { mode: 'all-matching' as const, search: 'a', filters: {}, excludedIds: new Set(['r1']) };
			const s = toggleRow(initial, 'r1');
			if (s.mode === 'all-matching') {
	...
	describe('togglePage', () => {
		...
		test('all-matching mode, page fully selected: adds all page ids to excludedIds', () => {
			const initial = { mode: 'all-matching' as const, search: 'q', filters: {}, excludedIds: new Set<string>() };
		...
		test('all-matching mode, page partially excluded: removes excluded ids (selecting them back)', () => {
			const initial = { mode: 'all-matching' as const, search: 'q', filters: {}, excludedIds: new Set(['a']) };
			const s = togglePage(initial, ['a', 'b']);
			if (s.mode === 'all-matching') {
	...
	describe('selectAllMatching', () => {
		test('switches to all-matching with snapshotted search, filters and empty excludedIds', () => {
			...
			if (s.mode === 'all-matching') {
				...
				expect(s.filters).toEqual({ programId: 'p1', recipientStatus: 'active' });
				expect(s.excludedIds.size).toBe(0);
			}
		});

		test('toggleRow preserves the snapshotted filters', () => {
			const start = selectAllMatching(emptySelection(), 'hello', { country: 'US' });
			const s = toggleRow(start, 'r1');
			if (s.mode === 'all-matching') {
	...
	describe('isRowSelected', () => {
		...
		test('all-matching mode: true iff id is NOT in excludedIds', () => {
			const s = { mode: 'all-matching' as const, search: 'q', filters: {}, excludedIds: new Set(['r1']) };
	...
	describe('getSelectedCount', () => {
		...
		test('all-matching mode: returns totalCount - excludedIds.size', () => {
			const s = { mode: 'all-matching' as const, search: 'q', filters: {}, excludedIds: new Set(['a', 'b']) };
		...
		test('all-matching mode: never returns negative when totalCount drifts below excludedIds.size', () => {
			const s = { mode: 'all-matching' as const, search: 'q', filters: {}, excludedIds: new Set(['a', 'b', 'c']) };
