...
export type RowFetcher = (page: number, pageSize: number, search: string) => Promise<{ ids: string[]; totalCount: number }>;
...
export async function resolveSelectionToIds(
	...
	while (collected.length < totalCount) {
		const result = await fetcher(page, pageSize, selection.search);
