...
export class MessagingWebhookService extends BaseService {
	...
	async handleStatusCallback(input: {
		...
		if (input.status === 'delivered') {
			// Flip to delivered only if it isn't already, atomically. Twilio can deliver duplicate or
			// out-of-order callbacks, so the row-level condition — not the value we read above — decides
			// whether this callback is the one that caused the transition. count === 1 guarantees the
			// deliveredCount increment happens exactly once even under concurrent callbacks.
			const flipped = await this.db.messageLog.updateMany({
				where: { id: existing.id, twilioStatus: { not: 'delivered' } },
				data: {
			...
			return this.resultOk({ updated: flipped.count === 1 });
		}
