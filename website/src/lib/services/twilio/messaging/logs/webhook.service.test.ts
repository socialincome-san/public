...
function makeService(opts: { existingLog?: { id: string; jobId: string; twilioStatus: string | null } }) {
	...
	const db = {
		messageLog: {
			...
			updateMany: ({ where, data }: { where: { twilioStatus?: { not: string } }; data: Record<string, unknown> }) => {
				// Model the atomic condition: the row matches only if its current status differs from the guard.
				const matches = opts.existingLog!.twilioStatus !== where.twilioStatus?.not;
				if (matches) {
					updates.push({ table: 'messageLog', data });
				}
