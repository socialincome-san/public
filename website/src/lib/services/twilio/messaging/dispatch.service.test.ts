...
import type { DispatchSendInput } from './twilio-messaging.types';
...
function makeService(opts: {
	...
	const twilioClient: MockTwilioClient = {
		messages: {
			create: jest.fn(async (_payload) => ({ sid: `SM${Math.random().toString(36).slice(2, 10)}` })),
		},
		content: {
			v1: {
				contents: jest.fn(() => ({
		...
		$transaction: async (fn: (tx: unknown) => Promise<unknown>) =>
			fn({
				...
				messageLog: {
					...
					findMany: async () => created.logs.map((d, i) => ({ id: `log_${i + 1}`, ...d })),
				},
			}),
		messagingJob: {
	...
	const twilioMessaging = {
		getContentTemplate: jest.fn(async () => ({
	...
	const service = new MessagingDispatchService(db as never, userService as never, twilioMessaging as never);
	...
	(service as unknown as { resolveContactIdsForType: (...args: unknown[]) => Promise<string[]> }).resolveContactIdsForType =
		async () => opts.contacts.map((c) => c.id);
