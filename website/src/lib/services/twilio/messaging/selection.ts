import type { PageCheckboxState, SelectionState } from './recipients.types';
...
export const toggleRow = (state: SelectionState, id: string): SelectionState => {
	...
	return { mode: 'all-matching', search: state.search, excludedIds };
};
...
export const selectAllMatching = (_state: SelectionState, search: string): SelectionState => ({
	mode: 'all-matching',
	search,
	excludedIds: new Set(),
});

export const clearSelection = (_state: SelectionState): SelectionState => emptySelection();
