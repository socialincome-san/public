...
export const createExpenseAction = async (input: ExpenseCreateInput) => {
	...
	const res = await services.write.expense.create(user.id, input);
...
export const updateExpenseAction = async (input: ExpenseUpdateInput) => {
	const user = await getAuthenticatedUserOrRedirect();
	const res = await services.write.expense.update(user.id, input);
	revalidatePath(REVALIDATE_PATH);
	return res;
};
...
export const getExpenseAction = async (id: string) => {
	const user = await getAuthenticatedUserOrRedirect();
	return services.read.expense.get(user.id, id);
};

export const getExpenseOptionsAction = async () => {
	const user = await getAuthenticatedUserOrRedirect();
	return services.read.organization.getOptions(user.id);
};
