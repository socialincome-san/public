...
export const createSurveyAction = async (input: SurveyCreateInput) => {
	...
	const result = await services.write.survey.create(user.id, input);
...
export const getSurveyAction = async (surveyId: string) => {
	const user = await getAuthenticatedUserOrThrow();
	return services.read.survey.get(user.id, surveyId);
};

export const updateSurveyAction = async (surveyId: string, input: SurveyUpdateInput) => {
	const user = await getAuthenticatedUserOrThrow();
	const result = await services.write.survey.update(user.id, surveyId, input);
	revalidatePath('/portal/management/surveys');
	return result;
};
...
export const previewSurveyGenerationAction = async () => {
	const user = await getAuthenticatedUserOrThrow();
	return services.read.survey.previewSurveyGeneration(user.id);
};

export const generateSurveysAction = async () => {
	const user = await getAuthenticatedUserOrThrow();
	const result = await services.write.survey.generateSurveys(user.id);
	revalidatePath('/portal/management/surveys');
	return result;
};
...
export const getByIdAndRecipient = async (surveyId: string, recipientId: string) => {
	...
	const result = await services.read.survey.getByIdAndRecipient(surveyId, recipientId);
	if (!result.success) {
...
export const saveChanges = async (surveyId: string, input: SurveyUpdateInput) => {
	...
	const result = await services.write.survey.saveChanges(surveyId, input);
	if (!result.success) {
