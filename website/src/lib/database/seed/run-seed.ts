...
import { focusesData } from './data/focuses.data';
import { localPartnerFocusesData } from './data/local-partner-focuses.data';
...
export const seedDatabase = async () => {
	await prisma.$transaction(async (tx) => {
		...
		await tx.programTargetFocus.deleteMany();
		...
		await tx.focus.createMany({ data: focusesData, skipDuplicates: true });
		await tx.programTargetFocus.createMany({ data: programTargetFocusesData, skipDuplicates: true });
		...
		await tx.localPartnerFocus.createMany({ data: localPartnerFocusesData, skipDuplicates: true });
