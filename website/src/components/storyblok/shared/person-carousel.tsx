		...
		<div className={cn('grid gap-8', hasSidebar ? 'lg:grid-cols-3 lg:items-center' : 'grid-cols-1')}>
			{hasSidebar ? (
				<div className="space-y-4 lg:col-span-1">
					{sidebar?.title ? <p className="text-foreground text-4xl font-bold break-words">{sidebar.title}</p> : null}
					{sidebar?.heading ? <h2 className="text-foreground text-4xl font-normal break-words">{sidebar.heading}</h2> : null}
					{sidebar?.description ? <p className="text-muted-foreground text-lg leading-7">{sidebar.description}</p> : null}
				</div>
			) : null}
