...
export const LocalPartnerTeaserCard = ({ localPartner, lang, region, viewDetailsLabel }: Props) => {
	...
	return (
		<article className="flex h-full w-full max-w-[305px] flex-col overflow-hidden rounded-xl bg-white p-3 shadow-[0px_4px_28px_0px_rgba(0,30,101,0.07)]">
			...
			<div className="flex flex-1 flex-col gap-3 px-2 pt-4 pb-2">
				...
				<NextLink
					href={`/${lang}/${region}/${NEW_WEBSITE_SLUG}/local-partners/${slug}`}
					className="text-foreground mt-auto inline-flex w-fit items-center gap-1.5 rounded-full bg-slate-100 py-1.5 pr-2 pl-3 text-xs leading-none font-bold transition-colors hover:bg-slate-200/80"
				>
					{viewDetailsLabel}
