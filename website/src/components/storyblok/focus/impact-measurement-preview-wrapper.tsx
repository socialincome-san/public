...
export const ImpactMeasurementPreviewWrapper = ({ focusId, lang, region, teaserButtonLabel, teaserText }: Props) => {
	...
	return (
		<div className={hasTeaser ? 'relative pb-24 sm:pb-16' : undefined}>
			<div className="bg-card after:bg-card relative h-96 overflow-hidden rounded-3xl shadow-[0_4px_6px_-4px_rgba(0,0,0,0.10),0_0_20px_0_rgba(0,0,0,0.05)] after:pointer-events-none after:absolute after:inset-x-0 after:bottom-0 after:h-1/2 after:mask-[linear-gradient(to_bottom,transparent_0%,black_100%)] after:backdrop-blur-[100px] after:content-[''] after:[-webkit-mask-image:linear-gradient(to_bottom,transparent_0%,black_100%)]">
			...
			{hasTeaser && (
				<div className="bg-card absolute inset-x-4 bottom-24 mx-auto flex max-w-3xl translate-y-1/2 flex-col items-stretch gap-4 rounded-[160px] px-6 py-6 shadow-[0_4px_100px_0_hsl(var(--border))] sm:bottom-16 sm:flex-row sm:items-center sm:justify-between sm:gap-16 sm:px-12 sm:py-10">
					{trimmedTeaserText && (
