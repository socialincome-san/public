...
export const Wallet = ({ variant = 'default', title, subtitle, badge, footerLeft, footerRight, href }: WalletProps) => {
  const content = (
    <Card
      ...
      style={{
        ['--slant-shift' as unknown]: '50px',
        ['--slant-width' as unknown]: '40px',
        ['--slant-height' as unknown]: '14px',
        ['--slant-position' as unknown]: 'calc(var(--slant-shift) + var(--slant-width))',
        ['--gradient' as unknown]: 'linear-gradient(to right, hsl(var(--gradient-card-from)), hsl(var(--gradient-card-to)))',
        ['--shadow-size' as unknown]: '11px',
        ['--stack-height' as unknown]: '28px',
        ['--wallet-front-bg' as unknown]: variant === 'default' ? 'var(--gradient)' : 'hsl(var(--card))',
        ['--wallet-front-box-shadow' as unknown]:
          variant === 'default'
            ? 'none'
            : '0 0 0 var(--shadow-size) rgba(255,255,255,0.5), 0 0 0 calc(2* var(--shadow-size)) rgba(255,255,255,0.3)',
        ['--wallet-back-bg' as unknown]: variant === 'default' ? 'var(--gradient)' : 'hsl(var(--secondary))',
        ['--wallet-cards-background' as unknown]: variant === 'default' ? 'hsl(var(--card))' : 'none',
        ['--wallet-cards-box-shadow' as unknown]:
          variant === 'default'
            ? '0 0 0 var(--shadow-size) rgba(255,255,255,0.5), 0 0 0 calc(2* var(--shadow-size)) rgba(255,255,255,0.3)'
            : 'none',
        ['--text-color' as unknown]: variant === 'default' ? 'hsl(var(--card))' : 'inherit',
      }}
    >
