  ...
  <DropdownMenu>
    <DropdownMenuTrigger asChild>
      <Button
        variant="ghost"
        className={twMerge(
          'relative rounded-md px-3 py-2 text-lg font-medium text-primary transition-colors duration-200 hover:bg-accent',
          className,
        )}
      >
        {active && <span className="absolute -bottom-1 left-0 h-1 w-full rounded-t-lg bg-primary" />}
        <span>Programs</span>
    ...
    <DropdownMenuContent align="start" className="w-56">
      ...
      <DropdownMenuItem asChild>
        <CreateProgramModal
          isAuthenticated
          trigger={
            <p className="flex cursor-pointer items-center gap-2 rounded-md p-2 text-sm font-medium transition-colors duration-200 hover:bg-accent">
