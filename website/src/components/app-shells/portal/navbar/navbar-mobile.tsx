...
export const NavbarMobile = ({ user }: { user: UserSession }) => {
  ...
  const ProfileName = () => (
    <div className="flex items-center space-x-3">
      ...
      <div className="text-left">
        ...
        <p className="text-xs text-muted-foreground">{user.activeOrganization?.name ?? 'No active organization'}</p>
      </div>
    </div>
  );
  ...
  return (
    <nav className="mb-4 lg:hidden">
      <div className={`flex h-14 items-center justify-between px-4 ${!isMenuOpen ? 'border-b border-border' : ''}`}>
        <Button
          variant="ghost"
          onClick={toggleMenu}
              ...
              <div className="mt-3 grid gap-1 p-2">
                {userMenuNavLinks.map(({ href, label }) => (
                  <Link
                    ...
                    href={href}
                    onClick={() => setIsMenuOpen(false)}
                    className="rounded-md px-2 py-2 font-medium text-muted-foreground hover:bg-accent"
                  >
                    {label}
                  </Link>
                ))}
                ...
                <button
                  onClick={logout}
                  className="rounded-md px-2 py-2 text-left font-medium text-destructive hover:bg-accent/50"
                >
                  Sign out
                </button>
              </div>
