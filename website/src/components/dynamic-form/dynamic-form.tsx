...
const buildZodSchema = (schemaDef: FormSchema): ZodObject<unknown> => {
...
const getDef = (key: keyof z.infer<typeof zodSchema>, zodSchema: z.ZodObject<unknown>, parentKey?: string) => {
  return parentKey ? zodSchema.shape[parentKey]?._def.shape()[key]?._def : zodSchema.shape[key]?._def;
};

// get Zod Type by Form Schema key
const getType = (key: keyof z.infer<typeof zodSchema>, zodSchema: z.ZodObject<unknown>, parentKey?: string): string => {
...
const isOptional = (key: keyof z.infer<typeof zodSchema>, zodSchema: z.ZodObject<unknown>, parentOption?: string) => {
        ...
        if (!isFormField(field)) {
          ...
          for (const [nestedName, nestedField] of Object.entries(field.fields)) {
            if (isFormField(nestedField) && nestedField.value != null) {
              form.setValue(`${name}.${nestedName}` as unknown, nestedField.value);
            }
          }
