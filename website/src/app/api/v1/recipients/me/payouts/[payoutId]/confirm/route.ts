...
export const POST = withAppCheck(async (request: NextRequest, { params }: { params: Params }) => {
	...
	if (!recipientResult.success) {
		console.warn('[POST /payouts/:id/confirm] Recipient resolution failed', {
	...
	if (!parsed.success) {
		console.warn('[POST /payouts/:id/confirm] Validation failed', {
	...
	const confirmResult = await updatePayoutStatusByRecipient(
	...
	if (!confirmResult.success) {
		console.error('[POST /payouts/:id/confirm] Update failed', {
	...
	console.info('[POST /payouts/:id/confirm] Payout confirmed', {
		payoutId,
		recipientId: recipientResult.data.id,
