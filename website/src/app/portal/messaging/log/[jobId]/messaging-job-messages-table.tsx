...
import type { MessagingJobDetailView, MessagingJobMessageRow } from '@/lib/services/twilio/messaging/twilio-messaging.types';
...
const twilioConsoleUrl = (accountSid: string, messageSid: string) =>
	`https://1console.twilio.com/account/${accountSid}/us1/messaging-logs/${messageSid}`;

const formatDate = (d: Date) =>
	d.toLocaleString(undefined, { year: 'numeric', month: 'short', day: '2-digit', hour: '2-digit', minute: '2-digit' });

export const MessagingJobMessagesTable = ({ jobId, messages, twilioAccountSid }: Props) => {
		...
		<div className="space-y-3">
								...
								<TableRow key={m.id}>
									...
									<TableCell>
										<div className="flex items-center gap-2">
											...
											{canLinkToTwilio && (
												<a
													href={twilioConsoleUrl(twilioAccountSid, m.twilioMessageSid!)}
													target="_blank"
			...
			<div className="text-muted-foreground flex items-center justify-between text-sm">
				...
				<div className="flex gap-2">
					{messages.page > 1 && (
						<Link href={`/portal/messaging/log/${jobId}?msgPage=${messages.page - 1}`} className="hover:underline">
							Prev
						</Link>
					)}
