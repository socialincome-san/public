	...
	const steps = [
		{
			...
			run: () => run(() => generateRegistrationCsvAction(providerId), 1),
		...
		},
		{
			...
			icon: <TableIcon className="h-4 w-4" />,
			variant: 'outline' as const,
			run: () => run(() => generatePayoutCsvAction(providerId, selectedDate), 2),
		...
		},
		{
			...
			variant: 'outline' as const,
			run: () => run(() => previewCurrentMonthPayoutsAction(providerId, selectedDate), 3),
		...
		},
		{
			...
			run: () => run(() => generateCurrentMonthPayoutsAction(providerId, selectedDate), 4),
		...
		<Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
			<DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[600px]">
				<DialogHeader className="flex-col items-start gap-0 border-b-0 pr-12">
					<DialogTitle className="text-pretty">Payout process — {providerName}</DialogTitle>
				</DialogHeader>
