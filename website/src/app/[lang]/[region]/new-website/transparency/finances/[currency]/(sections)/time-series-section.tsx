        ...
        <div className="flex h-64 items-end gap-2">
          {convertedRanges.map((range, index) => {
            ...
            return (
              ...
              >
                {isHovered && (
                  <div className="absolute -top-16 z-10 rounded-md border bg-popover px-3 py-2 text-sm text-popover-foreground shadow-md">
                ...
                <div className="relative flex h-48 w-full items-end justify-center">
                  <div
                    className={cn('w-full max-w-8 rounded-t bg-primary transition-all', isHovered && 'bg-primary/80')}
                    style={{ height: `${Math.max(heightPercent, 2)}%` }}
                  />
                </div>
                <span className="mt-2 text-xs text-muted-foreground">{range.label}</span>
              </div>
            );
          })}
        </div>
