...
export const IncomeInput = ({ translations }: { translations: { buttonText: string } }) => {
  ...
  const formSchema = z.object({
    value: z.string(),
    currency: z.enum(websiteCurrencies as unknown),
  });

  type FormSchema = z.infer<typeof formSchema>;
  const form = useForm<FormSchema>({
    resolver: zodResolver(formSchema),
    defaultValues: { value: '' as unknown, currency: '' as unknown },
  });
                ...
                <FormControl>
                  <Input
                    ...
                    className={classNames(
                      'h-full w-full border-none bg-background text-right text-5xl text-white opacity-40 focus-visible:opacity-100 focus-visible:ring-0',
