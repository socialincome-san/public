...
const STORYBLOK_MEDIA_PLACEHOLDER = '/assets/e2e-media-placeholder.svg';
...
const readReplayRecordings = (recordingKey: string) => {
	...
	return JSON.parse(replaceStoryblokMedia(restoreToken(raw))) as unknown;
};
...
const writeRecordings = (recordingKey: string, recordings: unknown) => {
	...
	fs.writeFileSync(fp, replaceStoryblokMedia(hideToken(json)));
};
