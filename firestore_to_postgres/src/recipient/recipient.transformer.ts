import { CreateRecipientInput } from '@socialincome/shared/src/database/recipient/recipient.types';
	...
	transform = async (input: FirestoreRecipient[]): Promise<CreateRecipientWithUser[]> => {
		return input.map((raw): CreateRecipientWithUser => {
			...
			return {
				user: {
					authUserId: null,
					email: email?.toLowerCase() ?? this.generateFallbackEmail(first_name, last_name),
				...
				partnerOrgName: organisation?.id.split('/').pop() ?? null,
			};
		});
	};
