...
export class PayoutForecastService extends BaseService {
	...
	async getPayoutForecast(programId: string, monthsAhead = 6): Promise<ServiceResult<PayoutForecastRow[]>> {
		try {
			const programResult = await this.programService.getProgramForForecast(programId);
			if (!programResult.success) return this.resultFail(programResult.error!);
			const programData = programResult.data;

			const recipientResult = await this.recipientService.getRecipientsForForecast(programId);
			if (!recipientResult.success) return this.resultFail(recipientResult.error!);
			const recipientList = recipientResult.data;
			...
			for (const recipient of recipientList) {
				const remainingPayments = this.calculateRemainingMonthlyPayments(
					recipient.startDate,
					programData.totalPayments,
				);
			...
			const baseCurrencyRate = exchangeRateResult.data[programData.payoutCurrency];
			...
			const payoutAmountUsd = this.round2((programData.payoutAmount / baseCurrencyRate) * usdCurrencyRate);
			...
			return this.resultOk(forecastRows);
		} catch (error) {
