...
const LastNameCol: AdditionalFieldDelegate<User> = {
  ...
  name: "Last Name",
  Builder: ({ entity }) => (
    <>{entity.values?.personal?.lastname}</>
  ),
  dependencies: ["personal"]
};
...
const ReferralCol: AdditionalFieldDelegate<User> = {
  ...
  name: "Referral",
  Builder: ({ entity }) => (
    <>{entity.values?.personal?.referral}</>
  ),
  dependencies: ["personal"]
};
